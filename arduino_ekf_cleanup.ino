...
void loop() {
    ...
    // Get rotation direction from estimated electrical velocity
    int rotation_direction = 0;
    // You might need to tune these thresholds (0.1f) based on actual motor speed and noise levels
    if (x_ekf(1) > 0.1f) {
        rotation_direction = 1; // CW
    } else if (x_ekf(1) < -0.1f) {
        rotation_direction = -1; // CCW
    }
