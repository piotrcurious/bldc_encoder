...
namespace Config {
    ...
    constexpr float LV_TH = 10.0f; // LOW_VELOCITY_THRESHOLD
    constexpr float HV_TH = 10.0f; // HIGH_VELOCITY_THRESHOLD
...
BLA::Matrix<NEKF, NEKF> Pe, Ie; // P_ekf, I_matrix_ekf
...
void uVH(float, unsigned long); // updateVelocityHistory
void uAH(float, unsigned long); // updateAngleHistory
...
void eSC() { // executeSensingCycle
    ...
    for (int i = 0; i < nST; i++) {
        ...
        delayMicroseconds(Config::EPWU);
        analogWrite(Config::CP, 0);
        // pinMode(Config::CP, INPUT); // Not strictly needed here, happens implicitly or later.
        // delayMicroseconds(Config::FSTU); // Not strictly needed here based on previous comment.
        r[0] += cAR(Config::PA);
        r[1] += cAR(Config::PB);
        r[2] += cAR(Config::PC);
    }
    for (int i = 0; i < 3; i++) r[i] /= nST;
    aMKF(r);
...
void uVH(float v, unsigned long ts) { // updateVelocityHistory, velocity, timestamp
    vH[vHI] = {0.0f, v, ts};
    vHI = (vHI + 1) % Config::VHS;
    if (vHI == 0) vHF = true;
}

void uAH(float a, unsigned long ts) { // updateAngleHistory, angle, timestamp
    aH[aHI] = {a, 0.0f, ts};
