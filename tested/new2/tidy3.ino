...
namespace Config {
    ...
    // NUM_SAMPLES_PER_PHASE will now be dynamic
    // int NUM_SAMPLES_PER_PHASE = 16; // Removed - now dynamic
    ...
    constexpr int MIN_EXCITATION_PULSE_WIDTH_US = 50;  // Min pulse width (at high velocity)
    constexpr int MAX_EXCITATION_PULSE_WIDTH_US = 1000; // Max pulse width (at zero/low velocity)
...
// NEW FUNCTION: Determine excitation pulse width based on velocity (corrected logic)
int getExcitationPulseWidth() {
    ...
    // At normalized_velocity = 0 (low speed), pulse_width = MAX_EXCITATION_PULSE_WIDTH_US
    // At normalized_velocity = 1 (high speed), pulse_width = MIN_EXCITATION_PULSE_WIDTH_US
    // This makes pulse width inversely proportional to velocity.
    int pulse_width = map(normalized_velocity * 1000, 0, 1000,
                          Config::MAX_EXCITATION_PULSE_WIDTH_US, Config::MIN_EXCITATION_PULSE_WIDTH_US);

    return constrain(pulse_width, Config::MIN_EXCITATION_PULSE_WIDTH_US, Config::MAX_EXCITATION_PULSE_WIDTH_US);
}

// NEW FUNCTION: Determine number of oversampling samples based on velocity (corrected logic)
...
void executeSensingCycle() {
    ...
    for (int i = 0; i < current_num_oversampling_samples; i++) {
        ...
        analogWrite(Config::COMMON_PIN, Config::EXCITATION_PWM_VALUE);
        delayMicroseconds(current_excitation_pulse_width_us); // Use velocity-based pulse width
    ...
    applyMultivariateKalmanFilter(readings, current_num_oversampling_samples); // Pass numSamples for potential KF adjustment
}
...
void applyMultivariateKalmanFilter(const int readings[3], int numSamples) {
    ...
    // Potentially adjust R_kf1 based on numSamples.
    // More samples -> theoretically less measurement noise, so R can be lower.
    // This is a subtle tuning point, test thoroughly.
    BLA::Matrix<N_STATES_KF1, N_STATES_KF1> current_R_kf1 = R_kf1; // Start with base R
    // Example: Make R scale inversely with sqrt of samples (similar to statistical noise reduction)
    // float noise_reduction_factor = sqrt((float)Config::MAX_OVERSAMPLING_SAMPLES / numSamples); // Or some other scaling
    // for (int i = 0; i < N_STATES_KF1; i++) {
    //     current_R_kf1(i,i) = Config::R_DIAGONAL_VALUE_KF1 / noise_reduction_factor;
    // }
    
    BLA::Matrix<N_STATES_KF1, N_STATES_KF1> S = P_kf1 + current_R_kf1; // Innovation covariance
...
void logData() {
    ...
    Serial.print(F("\tSmpl: ")); Serial.println(getNumOversamplingSamples()); // Log the current oversampling
