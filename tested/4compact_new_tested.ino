...
namespace Config {
    ...
    constexpr int DEFAULT_NUM_SAMPLES = 256; 

    // New parameters for dynamic oversampling
    constexpr int MIN_NUM_SAMPLES = 4;   // Minimum number of samples
    constexpr int MAX_NUM_SAMPLES = 256;  // Maximum number of samples
    // **MODIFIED THRESHOLDS FOR VELOCITY**
    constexpr float LOW_VELOCITY_THRESHOLD = 0.2f; // Velocity below which more samples are taken (rad/s)
    constexpr float HIGH_VELOCITY_THRESHOLD = 1.0f; // Velocity above which fewer samples are taken (rad/s)

    constexpr float ANGLE_UPDATE_DEADBAND = 0.02f, VELOCITY_STEP_THRESHOLD = 0.1f; // Reduced threshold
    ...
    constexpr float Q_ANGLE_EKF = 0.01f, Q_VEL_EKF = 10.1f, R_ANGLE_EKF = 0.01f; // Adjusted noise parameters
    
    // New parameters for improved tracking
    constexpr int VELOCITY_HISTORY_SIZE = 8;
    constexpr int ANGLE_HISTORY_SIZE = 5;
    constexpr int PHASE_HISTORY_SIZE = 10; // History for KF filtered phases
    constexpr float MIN_VELOCITY_CONSISTENCY = 0.13f; // Minimum velocity for consistent direction
    constexpr float VELOCITY_NOISE_THRESHOLD = 0.16f; // Threshold for velocity noise rejection
    constexpr float ANGLE_HISTORY_NOISE_THRESHOLD = 0.05f; // Threshold for angle consistency from phase history (radians)
