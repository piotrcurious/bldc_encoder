...
void applyEKF(float measured_angle_unwrapped); // EKF now takes an unwrapped measurement
void logData();
...
void loop() {
    ...
    // Calculate dt since last EKF update
    unsigned long now = micros();
    float dt = (now - prev_ekf_micros) / 1e6f; 
    if (dt == 0) { 
        dt = 1e-6f; // Ensure dt is never zero
    }
    // Update prev_ekf_micros immediately for the next loop iteration
    prev_ekf_micros = now; 

    // Perform EKF prediction to get the predicted angle for alignment
    predicted_ekf_angle_unwrapped = x_ekf(0) + x_ekf(1) * dt; // Angle is UNWRAPPED
    predicted_ekf_velocity = x_ekf(1); // Velocity is constant for prediction

    // 3. Prepare an UNWRAPPED measurement for the EKF, carefully aligned to the EKF's prediction.
    // This is the crucial step for robust phase alignment.
    // Calculate the difference between the *wrapped* raw measurement and the *wrapped* EKF prediction
    float wrapped_diff_to_prediction = normalizeAngle(current_raw_angle_from_kf - normalizeAngle(predicted_ekf_angle_unwrapped));
    
    // Add this wrapped difference to the *unwrapped* EKF prediction to get the aligned measurement
    float measured_angle_unwrapped_aligned = predicted_ekf_angle_unwrapped + wrapped_diff_to_prediction;

    // 4. Apply EKF: EKF now operates on and maintains an UNWRAPPED angle state.
    // Pass the precisely aligned unwrapped measurement to the EKF.
    ...
    if (Serial.available() > 0) {
        ...
        if (command == 'T') {
            ...
            P_ekf(0, 0) = 1.0f;
            P_ekf(1, 1) = 1.0f;
            prev_ekf_micros = micros(); // Reset EKF time reference
...
void updateUnwrappedElectricalAngle(float current_ekf_unwrapped_angle) {
    ...
    // This block is now primarily for maintaining previous_ekf_angle_wrapped_for_unwrapping
    // for consistency, though the main unwrapped_electrical_angle is directly x_ekf(0).
...
void updateAngleHistory(float angle, unsigned long timestamp) {
    angleHistory[angleHistoryIndex] = {angle, 0.0f, timestamp}; // angle is now unwrapped EKF angle
...
// EKF now takes an UNWRAPPED measured angle and predicts using UNWRAPPED angle
void applyEKF(float measured_angle_unwrapped) {
    // The dt calculation and prev_ekf_micros update are now handled in loop()
    // before calling applyEKF() to ensure predicted_ekf_angle_unwrapped is fresh.
    
    // Use the predicted values calculated in loop()
    ...
    A(0, 0) = 1; A(0, 1) = (micros() - (prev_ekf_micros - (long)(1e6f * (micros() - prev_ekf_micros) / 1e6f))) / 1e6f; // dt from global prev_ekf_micros updated in loop
    A(1, 0) = 0; A(1, 1) = 1;
    ...
    // Update Step: y is now the difference between two UNWRAPPED, aligned angles
    ...
    updateVelocityHistory(x_ekf(1), micros()); // Use current micros() as timestamp
    updateAngleHistory(x_ekf(0), micros()); // Use current micros() as timestamp
    ...
    if (is_moving && is_consistent_vel && is_consistent_phases && !is_stopping && (overall_measurement_confidence > 0.7f)) { 
        ...
        if (ekfLocked) {
            ...
            } else if (!is_moving && (micros() - lastNonZeroVelMicros > Config::ZERO_VEL_LOCK_TIMEOUT)) {
                should_unlock = true;
...
void logData() {
    ...
    // 14: Raw_Measured_Angle_Aligned (New: Aligned measurement sent to EKF for plotting)
    ...
    Serial.print(normalizeAngle(predicted_ekf_angle_unwrapped), 3); Serial.print(","); // Plot normalized prediction
    Serial.println(normalizeAngle(x_ekf(0)), 3); // Plot normalized EKF angle (actual EKF state)
