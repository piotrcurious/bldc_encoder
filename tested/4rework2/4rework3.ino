...
// RENAMED: previous_raw_electrical_angle -> previous_ekf_angle_wrapped for consistent unwrapping
float previous_ekf_angle_wrapped = 0.0f; 
...
// MODIFIED: Function now takes EKF's wrapped angle
void updateUnwrappedElectricalAngle(float current_ekf_angle_wrapped);
...
void setup() {
    ...
    // The KF state (x_hat_kf1, P_kf1) should already be initialized and stable from autoTuneExcitationPwm().
    // So, we can directly use it to initialize the EKF and unwrapped angle.
    float initial_kf_angle = getInferredElectricalAngle();
    x_ekf(0) = initial_kf_angle;
    ...
    // Initialize unwrapped angle using the EKF's initial angle
    first_angle_reading = true;
    updateUnwrappedElectricalAngle(x_ekf(0)); // This will set unwrapped_electrical_angle and previous_ekf_angle_wrapped
...
void loop() {
    ...
    // 2. Get the instantaneous electrical angle from KF filtered phases (this is the MEASUREMENT for EKF)
    float current_raw_angle_from_kf = getInferredElectricalAngle();

    // 3. Apply EKF: EKF now filters the current wrapped angle and estimates velocity
    applyEKF(current_raw_angle_from_kf); 

    // 4. Update the continuous, unwrapped electrical angle based on EKF's estimate
    // This is crucial for avoiding desynchronization.
    ...
    if (Serial.available() > 0) {
        ...
        if (command == 'T') {
            Serial.println(F("\nRe-tuning requested..."));
            autoTuneExcitationPwm(); // This will update x_hat_kf1 and P_kf1
            
            // Re-initialize EKF state and unwrapped angle
            float new_initial_kf_angle = getInferredElectricalAngle();
            x_ekf(0) = new_initial_kf_angle; // EKF angle is initialized from KF raw angle
            x_ekf(1) = 0.0f;
            P_ekf(0, 0) = 1.0f;
            P_ekf(1, 1) = 1.0f;
            prev_ekf_micros = micros();

            // Reset unwrapped angle tracking using the EKF's new initial angle
            first_angle_reading = true; 
            updateUnwrappedElectricalAngle(x_ekf(0)); 
...
void autoTuneExcitationPwm() {
    ...
    // Reset filters to a high-uncertainty state for auto-tuning
    x_hat_kf1.Fill(0.0f);
    P_kf1.Fill(0.0f);
    ...
    for (int iteration = 0; iteration < Config::PWM_CALIBRATION_ITERATIONS; iteration++) {
        ...
        applyMultivariateKalmanFilter(readings_tune); // Updates x_hat_kf1 and P_kf1
...
// MODIFIED: updateUnwrappedElectricalAngle now uses the EKF's wrapped angle (x_ekf(0))
// instead of the raw KF output.
void updateUnwrappedElectricalAngle(float current_ekf_angle_wrapped) {
    if (first_angle_reading) {
        unwrapped_electrical_angle = current_ekf_angle_wrapped;
        previous_ekf_angle_wrapped = current_ekf_angle_wrapped;
        first_angle_reading = false;
        return;
    }

    float angle_diff = current_ekf_angle_wrapped - previous_ekf_angle_wrapped;
