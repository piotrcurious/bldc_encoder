...
namespace Config {
    ...
    // NEW: Angle synchronization threshold for EKF measurement gating
    // If the difference between current raw angle and EKF predicted angle is > this,
    // EKF effectively ignores the measurement. Tune this carefully.
    constexpr float ANGLE_SYNC_THRESHOLD = PI_F * 0.75f; // e.g., 135 degrees
...
// MODIFIED: applyEKF now takes the wrapped difference as well for gating
...
void loop() {
    ...
    // Pass the precisely aligned unwrapped measurement AND the wrapped phase difference for gating.
...
// MODIFIED: EKF now takes an UNWRAPPED measured angle and the current wrapped difference for gating
void applyEKF(float measured_angle_unwrapped, float current_wrapped_diff) {
    // The dt calculation and predicted_ekf_angle_unwrapped/velocity are now handled in loop()
    ...
    // Calculate dt based on global prev_ekf_micros, which is updated in loop()
    ...
    // Base R_ekf determined by overall confidence
    float effective_R_ekf_value = Config::R_ANGLE_EKF_MIN + (1.0f - overall_measurement_confidence) * (Config::R_ANGLE_EKF_MAX - Config::R_ANGLE_EKF_MIN);
    
    // --- GATING MECHANISM: If angles are too far apart, drastically increase R_ekf ---
    if (fabsf(current_wrapped_diff) > Config::ANGLE_SYNC_THRESHOLD) {
        effective_R_ekf_value = Config::R_ANGLE_EKF_MAX * 100.0f; // Effectively ignore measurement
    }
    
    R_ekf(0, 0) = effective_R_ekf_value;
...
void logData() {
    ...
    // 15: Wrapped_Phase_Difference (New: For monitoring the gating condition)
