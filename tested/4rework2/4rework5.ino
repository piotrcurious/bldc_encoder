...
// This will now store the *wrapped* previous angle for internal unwrapping logic
// of the EKF's own angle, if needed (though now EKF's angle is inherently unwrapped).
// Retaining for the updateUnwrappedElectricalAngle's internal logic on x_ekf(0).
float previous_ekf_angle_wrapped_for_unwrapping = 0.0f; 
bool first_angle_reading_unwrapping = true; 

// This will now be directly equivalent to x_ekf(0) and will be the continuous angle
...
// MODIFIED: Function now takes EKF's current unwrapped angle
void updateUnwrappedElectricalAngle(float current_ekf_unwrapped_angle); 

void applyEKF(float measured_angle_unwrapped); // MODIFIED: EKF now takes an unwrapped measurement
void logData();
...
void loop() {
    ...
    // 3. Prepare an UNWRAPPED measurement for the EKF.
    // This is the crucial step for phase alignment.
    float measured_angle_unwrapped_aligned = current_raw_angle_from_kf;
    
    // Calculate how many 2*PI rotations are needed to bring current_raw_angle_from_kf 
    // into the same "unwrapped vicinity" as the EKF's current unwrapped_electrical_angle.
    float num_revolutions_diff = round( (unwrapped_electrical_angle - current_raw_angle_from_kf) / Config::TWO_PI_F );
    measured_angle_unwrapped_aligned = current_raw_angle_from_kf + num_revolutions_diff * Config::TWO_PI_F;

    // 4. Apply EKF: EKF now operates on and maintains an UNWRAPPED angle state.
    applyEKF(measured_angle_unwrapped_aligned); 

    // 5. Update the main continuous electrical angle variable directly from the EKF's unwrapped state.
    // This function also handles the internal wrapped angle tracking needed for its logic.
    ...
    if (Serial.available() > 0) {
        ...
        if (command == 'T') {
            ...
            // Re-initialize EKF state and unwrapped angle based on new KF output
            float new_initial_kf_angle_wrapped = getInferredElectricalAngle();
            x_ekf(0) = new_initial_kf_angle_wrapped; // EKF's angle is re-initialized to this wrapped value
            x_ekf(1) = 0.0f;
            P_ekf(0, 0) = 1.0f;
            P_ekf(1, 1) = 1.0f;
...
// MODIFIED: updateUnwrappedElectricalAngle now uses the EKF's unwrapped angle (x_ekf(0))
// It updates the global unwrapped_electrical_angle and also tracks a wrapped version of EKF's angle
// for its internal logic to correctly determine continuous angle accumulation.
void updateUnwrappedElectricalAngle(float current_ekf_unwrapped_angle) {
    // The global unwrapped_electrical_angle is directly the EKF's continuous angle
    unwrapped_electrical_angle = current_ekf_unwrapped_angle;

    // For internal tracking to correctly calculate angle differences for future unwrapping logic
    // (if any part of the system still needs wrapped angle comparison against this unwrapped angle)
    ...
    if (first_angle_reading_unwrapping) {
        ...
        // This differential unwrapping logic is primarily useful if unwrapped_electrical_angle
        // was derived from this. But now, unwrapped_electrical_angle is directly x_ekf(0).
        // This block primarily keeps previous_ekf_angle_wrapped_for_unwrapping updated for consistency
        // with any potential future use-cases or debugging where wrapped difference is needed.
        ...
        // previous_ekf_angle_wrapped_for_unwrapping should always track the wrapped version for next iteration's diff
        previous_ekf_angle_wrapped_for_unwrapping = current_ekf_wrapped;
    }
}
...
void updateAngleHistory(float angle, unsigned long timestamp) {
    angleHistory[angleHistoryIndex] = {angle, 0.0f, timestamp}; // angle will now be unwrapped EKF angle
...
HistoricalPhaseStats getHistoricalPhaseStats() {
    ...
    stats.inferredAngleAtCurrentTime = normalizeAngle(inferred_angle_at_current_time); // Still return normalized for this candidate
...
// MODIFIED: EKF now takes an UNWRAPPED measured angle
void applyEKF(float measured_angle_unwrapped) {
    ...
    float angle = x_ekf(0), velocity = x_ekf(1); // angle is now UNWRAPPED

    // Predict: Predicted angle is also UNWRAPPED
    float predicted_angle = angle + velocity * dt;
    ...
    historical_inferred_angle = historicalStats.inferredAngleAtCurrentTime; // This is still wrapped for display
    ...
    // Update Step: y is now the difference between two UNWRAPPED angles
    float y = measured_angle_unwrapped - predicted_angle; 
    // No normalizeAngle(y) here, as both inputs are unwrapped and aligned to the same "revolution"
    ...
    // updateAngleHistory takes EKF's unwrapped angle
...
void logData() {
    ...
    Serial.print(unwrapped_electrical_angle, 3); Serial.print(","); // Unwrapped EKF angle
    ...
    Serial.print(historical_inferred_angle, 3); Serial.print(","); // Historical angle is still normalized (-PI, PI] for display
    Serial.print(historical_inferred_velocity, 4); Serial.print(","); 
    Serial.print(getClarkeMagnitude(), 2); Serial.print(","); 
    Serial.println(R_ekf(0,0), 2); 
