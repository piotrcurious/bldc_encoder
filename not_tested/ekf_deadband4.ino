...
void loop() {
    ...
    estimated_mechanical_steps = round(unwrapped_electrical_angle / Config::TWO_PI * Config::ELECTRICAL_STEPS_PER_REV);
...
float getInferredElectricalAngle() {
    ...
    return (angle < 0.0f) ? angle + Config::TWO_PI : angle; // Map to [0, 2PI]
}
...
void applyEKF(float inferred_angle_measurement) {
    ...
    x_ekf(0) = fmodf(x_ekf(0), Config::TWO_PI);
    if (x_ekf(0) < 0.0f) x_ekf(0) += Config::TWO_PI;
    ...
    unwrapped_electrical_angle = x_ekf(0) + revolution_count * Config::TWO_PI;
}
 ...
 */
float normalizeAngle(float angle) {
    angle = fmodf(angle + PI, Config::TWO_PI);
    if (angle < 0.0f) {
        angle += Config::TWO_PI;
    }
    return angle - PI;
