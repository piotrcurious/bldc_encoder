...
// Removed previousHallState and lastStateChangeMillis, as EKF handles state transitions

const int NUM_SAMPLES = 16;
// —————————————————————————————
// Thresholds & gating for stationary suppression
const float VELOCITY_DEADBAND         = 0.2f;   // rad/s: below this we consider "no motion"
const int   DIRECTION_STABLE_CYCLES   = 3;      // how many loops the sign must hold
// —————————————————————————————

// Removed VOLTAGE_THRESHOLD and MIN_STATE_SPREAD as EKF will handle state interpretation
// The filtering layer still uses these for noise settings, but the state logic doesn't.
...
// Tune these values: small Q_angle for confident angle, larger Q_vel for responsive velocity
const float Q_ANGLE_EKF = 0.001f; // Rad^2/s^2 (e.g., 0.001 for low process noise on angle)
const float Q_VEL_EKF = 0.1f;    // (rad/s)^2/s^2 (e.g., 0.1 for allowing velocity changes)
...
// Tune this: How noisy is your atan2 calculation?
const float R_ANGLE_EKF = 0.1f; // (rad)^2 (e.g., 0.1 for moderately noisy measurement)
...
void applyEKF(float inferred_angle_measurement) {
    ...
    if (dt == 0) return; // Avoid division by zero if called too fast
    ...
    // Optional: Log EKF diagnostics
    // Serial.print("EKF Angle: "); Serial.print(x_ekf(0) * 180.0 / PI, 2);
    // Serial.print("\tEKF Vel: "); Serial.print(x_ekf(1), 2);
    // Serial.print("\tInferred Ang: "); Serial.println(inferred_angle_measurement * 180.0 / PI, 2);
}
...
void loop() {
    ...
    executeSensingCycle();                        // update g_valA/B/C
    float inferred_angle = getInferredElectricalAngle();
    applyEKF(inferred_angle);

    // Convert to continuous Hall‐step estimate
    long raw_steps = round(x_ekf(0) / (2.0f * PI) * 6.0f);

    // 1) Compute instantaneous velocity & direction
    float vel = x_ekf(1); 
    int  instant_dir = 0;
    if      (vel >  VELOCITY_DEADBAND) instant_dir =  1;
    else if (vel < -VELOCITY_DEADBAND) instant_dir = -1;
    // else instant_dir stays 0

    // 2) Hysteresis: require stable sign for N loops
    static int dir_history[DIRECTION_STABLE_CYCLES] = {0};
    static int hist_idx = 0;
    dir_history[hist_idx++] = instant_dir;
    if (hist_idx >= DIRECTION_STABLE_CYCLES) hist_idx = 0;

    // check if all entries agree
    int stable_dir = dir_history[0];
    for (int i = 1; i < DIRECTION_STABLE_CYCLES; ++i) {
        if (dir_history[i] != stable_dir) {
            stable_dir = 0;
            break;
        }
    }

    // 3) Only update mechanical_steps if we have a stable nonzero direction
    static long last_reported_steps = 0;
    if (stable_dir != 0) {
        estimated_mechanical_steps = raw_steps;
        last_reported_steps = raw_steps;
    } else {
        // hold at last value when stationary
        estimated_mechanical_steps = last_reported_steps;
    }

    // 4) Output for Serial Plotter (now cleaned up)
    Serial.print(g_valA, 2); Serial.print('\t');
    Serial.print(g_valB, 2); Serial.print('\t');
    Serial.print(g_valC, 2); Serial.print('\t');
    Serial.print(inferred_angle * 180.0/PI, 2); Serial.print('\t');
    Serial.print(x_ekf(0) * 180.0/PI, 2);       Serial.print('\t');
    Serial.print(vel, 2);                       Serial.print('\t');
    Serial.print(estimated_mechanical_steps);   Serial.print('\t');
    Serial.println(stable_dir);

    // … rest of your re‐tuning logic …
#endif
}
