...
const float ZERO_VEL_LOCK_TIMEOUT = 500e3f;    // micros
...
const float R_ANGLE_EKF = 0.5f; // (rad)^2 (e.g., 0.1 for moderately noisy measurement)
...
void applyEKF(float inferred_angle_measurement) {
    ...
    // Deadband check
    if (fabsf(y) < ANGLE_UPDATE_DEADBAND && fabsf(x_ekf(1)) < VELOCITY_STEP_THRESHOLD) {
        // Skip update, maintain locked if stationary
        if (!ekfLocked) {
...
void loop() {
    executeSensingCycle();
    float inferred = getInferredElectricalAngle();
    applyEKF(inferred);

    float vel = x_ekf(1);
    if (fabsf(vel) > VELOCITY_STEP_THRESHOLD) {
        estimated_mechanical_steps = round(x_ekf(0)/(2*PI)*6.0f);
    }

    int rotation_direction = (vel > VELOCITY_STEP_THRESHOLD) ? 1 :
                              (vel < -VELOCITY_STEP_THRESHOLD) ? -1 : 0;

    // Output for Serial Plotter:
    // Filtered A, Filtered B, Filtered C, Inferred Angle (deg), EKF Angle (deg), EKF Velocity (rad/s), Estimated Mechanical Steps, Rotation Direction
    ...
    Serial.print("\t");
    Serial.print(x_ekf(1), 2);                    // EKF estimated velocity in rad/s
    Serial.print("\t");
